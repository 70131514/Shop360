...
### TC-FR1.1-NEG-002: Sign Up with Weak Password
- **Implemented in code**: Yes — Auth via Firebase in `authService` + auth screens + AppNavigator gating
- **ID**: TC-FR1.1-NEG-002
- **Priority**: Medium
- **Description**: Verify system rejects sign up with password less than 6 characters
- **Reference**: FR1.1
- **Users**: Guest User
- **Pre-Requisites**: 
  - A. App is installed and launched
...
- **Steps**: 
  ...
  - C. Enter password with less than 6 characters (e.g., Test1)
  - D. Enter name
  - E. Tap "Sign Up" button
- **Input**: Email: testuser@example.com, Password: Test1, Name: Test User
- **Expected Result**: Error message displayed indicating password must be at least 6 characters, sign up fails
- **Status**: Not Tested

### TC-FR1.1-NEG-003: Sign Up with Existing Email
- **Implemented in code**: Yes — Auth via Firebase in `authService` + auth screens + AppNavigator gating
- **ID**: TC-FR1.1-NEG-003
- **Priority**: High
- **Description**: Verify system rejects sign up with already registered email
- **Reference**: FR1.1
- **Users**: Guest User
- **Pre-Requisites**: 
  - A. App is installed and launched
  - B. User is on Signup screen
  - C. Email address already exists in system
- **Steps**: 
  - A. Navigate to Signup screen
  - B. Enter existing email address
  - C. Enter valid password
  - D. Enter name
  - E. Tap "Sign Up" button
- **Input**: Email: existing@example.com, Password: Test123!, Name: Test User
- **Expected Result**: Error message displayed indicating email already in use, sign up fails
- **Status**: Not Tested
...
### TC-FR1.2-NEG-002: Sign In with Non-Existent Email
- **Implemented in code**: Yes — Auth via Firebase in `authService` + auth screens + AppNavigator gating
- **ID**: TC-FR1.2-NEG-002
- **Priority**: High
- **Description**: Verify system rejects sign in with unregistered email
- **Reference**: FR1.2
- **Users**: Guest User
- **Pre-Requisites**: 
  - A. App is installed and launched
...
- **Expected Result**: Error message displayed indicating user not found, sign in fails
- **Status**: Not Tested
...
### TC-FR4.1-POS-002: Add Product to Cart (Guest User)
- **Implemented in code**: Yes — Cart via `cartService` + `CartScreen` (Firestore for verified users)
- **ID**: TC-FR4.1-POS-002
- **Priority**: High
- **Description**: Verify guest user can add product to cart
- **Reference**: FR4.1
- **Users**: Guest User
- **Pre-Requisites**: 
...
- **Steps**: 
  ...
  - B. Tap "Add to Cart" button
- **Input**: Product ID: prod123, Quantity: 1
- **Expected Result**: Product is added to guest cart (local storage), cart count increases
- **Status**: Not Tested
...
### TC-FR4.1-NEG-002: Add Product to Cart with Unverified Email
- **Implemented in code**: Yes — Cart via `cartService` + `CartScreen` (Firestore for verified users)
- **ID**: TC-FR4.1-NEG-002
- **Priority**: High
- **Description**: Verify unverified user cannot add product to cart
- **Reference**: FR4.1
- **Users**: Unverified User
- **Pre-Requisites**: 
  - A. User is signed in but email not verified
  - B. User is on Product Details screen
- **Steps**: 
  - A. Navigate to Product Details screen
  - B. Tap "Add to Cart" button
- **Input**: Product ID: prod123
- **Expected Result**: Error message displayed prompting user to verify email, product not added to cart
- **Status**: Not Tested
...
### TC-FR10.4-POS-002: Rotate Model in AR
- **Implemented in code**: Yes — AR via `ARViewScreen` + Viro + `ModelPlacementARScene`
- **ID**: TC-FR10.4-POS-002
- **Priority**: Low
- **Description**: Verify user can rotate 3D model in AR view
- **Reference**: FR10.4
- **Users**: Any User (Guest or Authenticated)
- **Pre-Requisites**: 
  - A. User is on AR View screen
  - B. 3D model is placed in AR
- **Steps**: 
  - A. Use rotate gesture on model
- **Input**: Rotation: 45 degrees
- **Expected Result**: Model rotates based on gesture
- **Status**: Not Tested

### TC-FR10.4-POS-003: Reset Model Position
- **Implemented in code**: Yes — AR via `ARViewScreen` + Viro + `ModelPlacementARScene`
- **ID**: TC-FR10.4-POS-003
- **Priority**: Low
- **Description**: Verify user can reset model position in AR view
- **Reference**: FR10.4
- **Users**: Any User (Guest or Authenticated)
- **Pre-Requisites**: 
  - A. User is on AR View screen
  - B. 3D model is placed in AR
- **Steps**: 
  - A. Tap "Reset Pos" button
- **Input**: None
- **Expected Result**: Model position is reset to original placement
- **Status**: Not Tested
...
### TC-FR13.3-POS-002: Update Order Status to Delivered
- **Implemented in code**: Yes — Admin order management via `adminService` + `AdminOrdersScreen`/`AdminOrderDetailScreen`
- **ID**: TC-FR13.3-POS-002
- **Priority**: High
- **Description**: Verify admin can update order status from shipped to delivered
- **Reference**: FR13.3
- **Users**: Admin User
- **Pre-Requisites**: 
  - A. User is signed in as admin
  - B. User is viewing order details
  - C. Order status is "shipped"
- **Steps**: 
  - A. Navigate to Order Details screen
  - B. Tap "Update Status" button
  - C. Select "Delivered" status
  - D. Confirm the update
- **Input**: Order ID: order123, New Status: "delivered"
- **Expected Result**: Order status is updated to "delivered", timeline entry is added
- **Status**: Not Tested
...
### TC-FR13.3-NEG-002: Update Delivered Order Status
- **Implemented in code**: Yes — Admin order management via `adminService` + `AdminOrdersScreen`/`AdminOrderDetailScreen`
- **ID**: TC-FR13.3-NEG-002
- **Priority**: Medium
- **Description**: Verify system prevents updating status of delivered orders
- **Reference**: FR13.3
- **Users**: Admin User
- **Pre-Requisites**: 
  - A. User is signed in as admin
  - B. User is viewing order details
  - C. Order status is "delivered"
- **Steps**: 
  - A. Navigate to Order Details screen
  - B. Tap "Update Status" button
  - C. Try to select any status other than "delivered"
- **Input**: Order ID: order123, Attempted Status: "processing"
- **Expected Result**: Error message displayed indicating delivered orders cannot be updated, status remains unchanged
- **Status**: Not Tested
...
### TC-FR13.4-NEG-002: Approve Cancellation Without Request
- **Implemented in code**: Yes — Admin order management via `adminService` + `AdminOrdersScreen`/`AdminOrderDetailScreen`
- **ID**: TC-FR13.4-NEG-002
- **Priority**: Medium
- **Description**: Verify system prevents approving cancellation when no request exists
- **Reference**: FR13.4
- **Users**: Admin User
- **Pre-Requisites**: 
  - A. User is signed in as admin
...
### TC-FR14.3-POS-002: Update User Role to User
- **Implemented in code**: Yes — Admin user management via `adminService` + `AdminUsersScreen`/`AdminUserDetailScreen`
- **ID**: TC-FR14.3-POS-002
- **Priority**: High
- **Description**: Verify admin can update a user's role from admin to user
- **Reference**: FR14.3
- **Users**: Admin User
- **Pre-Requisites**: 
  - A. User is signed in as admin
  - B. User is viewing user details
  - C. Target user role is "admin"
- **Steps**: 
  - A. Navigate to User Details screen
  - B. Tap "Change Role" button
  - C. Select "user" role
  - D. Confirm the change
- **Input**: User ID: user456, New Role: "user"
- **Expected Result**: User role is updated to "user", admin access is revoked
- **Status**: Not Tested
...
### TC-FR16.1-NEG-002: Submit Ticket with Empty Message
- **Implemented in code**: Yes — Support tickets via `ticketService` + Help/MyTickets screens
- **ID**: TC-FR16.1-NEG-002
- **Priority**: Medium
- **Description**: Verify system prevents submitting ticket with empty message
- **Reference**: FR16.1
- **Users**: Authenticated User
- **Pre-Requisites**: 
  - A. User is signed in
...
- **Steps**: 
  - A. Navigate to Help & Support screen
  - B. Tap "Submit Ticket" button
  - C. Leave message field empty
  - D. Try to submit
- **Input**: Message: ""
- **Expected Result**: Error message displayed indicating message is required, ticket not created
- **Status**: Not Tested
...
### TC-FR17.3-POS-002: Update Inquiry Status to Resolved
- **Implemented in code**: Yes — Admin inquiries via `ticketService` + `AdminInquiriesScreen`
- **ID**: TC-FR17.3-POS-002
- **Priority**: High
- **Description**: Verify admin can update ticket status to resolved
- **Reference**: FR17.3
- **Users**: Admin User
- **Pre-Requisites**: 
  - A. User is signed in as admin
  - B. User is viewing ticket details
  - C. Ticket status is "in-progress" or "open"
- **Steps**: 
  - A. Navigate to Ticket Details screen
  - B. Tap "Update Status" button
  - C. Select "resolved" status
  - D. Confirm the update
- **Input**: Ticket ID: ticket123, New Status: "resolved"
- **Expected Result**: Ticket status is updated to "resolved", timeline entry is added
- **Status**: Not Tested
...
### TC-FR18.1-POS-002: Change Theme to Light Mode
- **Implemented in code**: Yes — Settings via Theme/Font contexts + Settings/Policy/Terms screens
- **ID**: TC-FR18.1-POS-002
- **Priority**: Low
- **Description**: Verify user can switch to light theme
- **Reference**: FR18.1
- **Users**: All Users
- **Pre-Requisites**: 
  - A. User is signed in or using as guest
  - B. User is on Settings screen
  - C. Current theme is dark mode
- **Steps**: 
  - A. Navigate to Settings screen
  - B. Tap theme toggle or select light mode
- **Input**: Theme: "light"
- **Expected Result**: App theme changes to light mode, all screens reflect light theme colors
- **Status**: Not Tested
...
### TC-FR18.2-POS-002: Decrease Font Size
- **Implemented in code**: Yes — Settings via Theme/Font contexts + Settings/Policy/Terms screens
- **ID**: TC-FR18.2-POS-002
- **Priority**: Low
- **Description**: Verify user can decrease font size
- **Reference**: FR18.2
- **Users**: All Users
- **Pre-Requisites**: 
  - A. User is signed in or using as guest
...
- **Steps**: 
  - A. Navigate to Settings screen
  - B. Adjust font size slider to decrease
- **Input**: Font Size: "Small"
- **Expected Result**: Font size decreases across the app
- **Status**: Not Tested
